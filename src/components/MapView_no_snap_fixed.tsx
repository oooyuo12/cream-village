...
function createPathSignature(path: LatLng[]): string {
  if (path.length === 0) {
    return "empty";
  }
...
function MapCenterController({
  ...
  useEffect(() => {
    ...
    const autoFitSignature = `${selectedRouteId}:${pathSignature}`;
...
export const MapView: React.FC<MapViewProps> = ({
  ...
  const displayPath = useMemo(
    () => mergeDisplayPaths(outboundDisplayPath, returnDisplayPath),
    [outboundDisplayPath, returnDisplayPath]
  );

  const displayPathSignature = useMemo(
